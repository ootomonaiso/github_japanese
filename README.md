...
GitHubの英語UIをリアルタイムに日本語へ置き換えるChrome拡張機能です。シンプルなトグルと説明付きポップアップで、翻訳のON/OFFやワンクリック翻訳を素早く操作できます。
...
- 新規リポジトリ作成など長文の説明が多い画面では、翻訳プロファイルを自動読み込みしてより自然な文章に置換
...
- **🔧 開発モード**: 未翻訳の英語テキストを自動収集し、コンソールで確認できます（翻訳辞書の拡充に便利）。

### 開発モードの使い方
1. ポップアップで「🔧 開発モード」をONにします。
2. GitHubの好きなページを開いて操作します。
3. 開発者コンソール（F12）を開き、以下のコマンドが使えます:
   - `ghjpDevTools.showUntranslated()` - 未翻訳テキストの一覧を表示
   - `ghjpDevTools.exportJSON()` - `translations.json`に追加できるJSON形式で出力
   - `ghjpDevTools.highlightUntranslated()` - 未翻訳要素を赤枠でハイライト
   - `ghjpDevTools.clear()` - 収集した未翻訳リストをクリア
4. 出力されたJSONを`translations.json`にコピーして翻訳を追加できます。

## 翻訳プロファイル
`profiles/` ディレクトリに画面ごとの翻訳セットをJSONで定義できます。`content.js` が現在のURLに応じて必要なプロファイルを読み込み、`translations.json` とマージします。長い説明文や段落をまとめて翻訳したいときに活用してください。
